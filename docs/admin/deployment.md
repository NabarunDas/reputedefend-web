...
| Environment | Existing flags and secrets unchanged | No secrets required for foundation |
...
2. Use the PR branch for its first Preview. Enable deployment protection and isolated test environment configuration. The foundation has no DB or mail connection.
3. Verify `/login` shows sign-in unavailable; `/` and `/clients/example` redirect there; `/api/clients` returns 401 JSON; `/robots.txt` disallows all crawlers. Verify noindex, no-store, nosniff, no-referrer and frame protection headers.
...
6. Verify TLS and host routing. Configure exact Auth origins/redirects only when OTP is implemented. Do not add wildcard customer/admin redirects or cross-subdomain cookies.
7. Keep staff access disabled until OTP and membership checks pass their own acceptance gate. Foundation deployment is not an operational admin launch.

## Before the OTP stage
...
- Confirm named Owner and backup identities. Email possession alone will not grant a staff role.
...
Revert the foundation PR or redeploy the admin project's previous version. No database rollback is required. The marketing app and existing intake functions are not migrated. If admin DNS was added, remove only that subdomain mapping when rolling back; leave apex/www and email DNS intact.
