...
describe("evidence workspace SQL", () => {
  ...
  it("links new documents only to OPEN requests for the same case", async () => {
    ...
    expect(await rpc("admin_evidence_begin_v1", [token, key(), caseId, null, "invoice.pdf", "application/pdf", 1024, "Supporting invoice", "test-evidence", fulfilled.id])).toEqual({ status: "conflict" })
    ...
    expect(await rpc("admin_evidence_begin_v1", [token, key(), caseId, null, "invoice.pdf", "application/pdf", 1024, "Supporting invoice", "test-evidence", cancelled.id])).toEqual({ status: "conflict" })
