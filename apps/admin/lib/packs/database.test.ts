...
async function rpc(name: string, args: unknown[] = []) {
  return (await db.query<{ value: any }>(`select public.${name}(${args.map((_, i) => `$${i + 1}`).join(",")}) as value`, args)).rows[0].value
}
...
const begin = (filename = "invoice.pdf", type = "application/pdf", document: string | null = null, id = caseId) =>
  rpc("admin_evidence_begin_v1", [token, key(), id, document, filename, type, 1024, "Supporting invoice", "test-evidence", null])
...
const caseCmd = (operation: string, data: Record<string, unknown>, version: number, request = key()) =>
  rpc("admin_case_command_v1", [token, request, caseId, version, operation, { note, ...data }])
...
describe("prepared pack SQL", () => {
  ...
  it("creates a draft pack idempotently and conflicts on a stale record version", async () => {
    ...
    const second = await packCmd("create")
    expect(second.packNumber).toBe(2)
    expect((await packCmd("add_item", { versionId: crypto.randomUUID() }, created.id, created.recordVersion + 1)).status).toBe("conflict")
  })
  ...
  it("adds only clean valid ACCEPTED versions from the same case and derives snapshots in the database", async () => {
    ...
    expect((await packCmd("add_item", { versionId: unreviewed.versionId }, pack.id, next.packs[0].recordVersion)).status).toBe("denied")
    const denied = [
      await packCmd("add_item", { versionId: scanning.versionId }, pack.id, next.packs[0].recordVersion),
      await packCmd("add_item", { versionId: threats.versionId }, pack.id, next.packs[0].recordVersion),
      await packCmd("add_item", { versionId: invalid.versionId }, pack.id, next.packs[0].recordVersion),
      await packCmd("add_item", { versionId: other.versionId }, pack.id, next.packs[0].recordVersion),
    ]
  ...
  it("reorders and removes draft items and rejects duplicate positions", async () => {
    ...
    expect(detail.packs[0].items.map((item: { versionId: string }) => item.versionId)).toEqual([first.versionId, second.versionId])
    ...
    expect(detail.packs[0].items.map((item: { versionId: string }) => item.versionId)).toEqual([second.versionId, first.versionId])
  ...
  it("approves a non-empty pack, supersedes the previous approved pack and does not change the case workflow", async () => {
    ...
    const empty = await packCmd("create")
    expect((await packCmd("approve", { note: packNote, confirmed: true }, empty.id, empty.recordVersion)).status).toBe("denied")
    ...
    const second = await packCmd("create")
    const after = await rpc("admin_prepared_pack_case_v1", [token, caseId])
    const draft = after.packs.find((pack: { status: string }) => pack.status === "DRAFT")
    await packCmd("add_item", { versionId: replacement.versionId }, draft.id, draft.recordVersion)
    const readySecond = await rpc("admin_prepared_pack_case_v1", [token, caseId])
    const draftNow = readySecond.packs.find((pack: { status: string }) => pack.status === "DRAFT")
    expect(await packCmd("approve", { note: packNote, confirmed: true }, draftNow.id, draftNow.recordVersion)).toMatchObject({ packStatus: "APPROVED" })
    const packs = await rpc("admin_prepared_pack_case_v1", [token, caseId])
    expect(packs.packs.find((pack: { id: string }) => pack.id === first.id).status).toBe("SUPERSEDED")
    expect(packs.packs.filter((pack: { status: string }) => pack.status === "APPROVED")).toHaveLength(1)
    ...
    await caseCmd("plan", { track: "GUIDED", priority: "NORMAL", assigned: true, nextAction: "Review the request", due: null, firstResponseDue: null }, detail.version)
    let current = await rpc("admin_case_detail_v1", [token, caseId, null])
    await caseCmd("transition", { target: "ASSESSMENT_READY", nextAction: "Follow up with customer", due: "2026-12-01T12:00:00Z" }, current.version)
    current = await rpc("admin_case_detail_v1", [token, caseId, null])
    await caseCmd("transition", { target: "SERVICE_SELECTION", nextAction: "Follow up with customer", due: "2026-12-01T12:00:00Z" }, current.version)
    current = await rpc("admin_case_detail_v1", [token, caseId, null])
    await caseCmd("transition", { target: "PAYMENT_REQUIRED", nextAction: "Follow up with customer", due: "2026-12-01T12:00:00Z" }, current.version)
    current = await rpc("admin_case_detail_v1", [token, caseId, null])
    expect(await caseCmd("transition", { target: "PREPARATION", nextAction: "Follow up with customer", due: "2026-12-01T12:00:00Z" }, current.version)).toEqual({ status: "prerequisite" })
    expect(await caseCmd("transition", { target: "READY_TO_SUBMIT", nextAction: "Follow up with customer", due: "2026-12-01T12:00:00Z" }, current.version)).toEqual({ status: "denied" })
    current = await rpc("admin_case_detail_v1", [token, caseId, null])
    ...
    current = await rpc("admin_case_detail_v1", [token, caseId, null])
    expect(await caseCmd("transition", { target: "READY_TO_SUBMIT", nextAction: "Follow up with customer", due: "2026-12-01T12:00:00Z" }, current.version)).toEqual({ status: "prerequisite" })
  })
  ...
  it("marks only affected APPROVED packs STALE when included evidence changes", async () => {
    ...
    const draft = listed.packs.find((row: { status: string }) => row.status === "DRAFT")
    await packCmd("add_item", { versionId: first.versionId }, draft.id, draft.recordVersion)
    const draftNow = (await rpc("admin_prepared_pack_case_v1", [token, caseId])).packs.find((row: { status: string }) => row.status === "DRAFT")
    await packCmd("approve", { note: packNote, confirmed: true }, draftNow.id, draftNow.recordVersion)
    const newer = await accepted("v2.pdf", "application/pdf", first.documentId)
    await rpc("admin_evidence_review_v1", [token, key(), caseId, newer.versionId, (await rpc("admin_evidence_version_v1", [token, caseId, newer.versionId])).recordVersion, "accept", "Newer accepted version for the same document.", null])
    expect((await rpc("admin_prepared_pack_case_v1", [token, caseId])).packs.find((row: { id: string }) => row.id === draftNow.id).status).toBe("STALE")

    const scanDoc = await accepted("scan.pdf")
    const scanPack = await packCmd("create")
    const scanDraft = (await rpc("admin_prepared_pack_case_v1", [token, caseId])).packs.find((row: { status: string }) => row.status === "DRAFT")
    await packCmd("add_item", { versionId: scanDoc.versionId }, scanDraft.id, scanDraft.recordVersion)
    const scanReady = (await rpc("admin_prepared_pack_case_v1", [token, caseId])).packs.find((row: { status: string }) => row.status === "DRAFT")
    await packCmd("approve", { note: packNote, confirmed: true }, scanReady.id, scanReady.recordVersion)
    ...
    expect((await rpc("admin_prepared_pack_case_v1", [token, caseId])).packs.find((row: { id: string }) => row.id === scanReady.id).status).toBe("STALE")
  })
