...
import { enquiryArgs, type EnquiryOperation } from "./validation"
const reply=(message:string,status:number,id?:string)=>NextResponse.json({message,...(id?{id}:{})},{status,headers:privateResponseHeaders})
const rpcs={create:"admin_enquiry_create_v1",triage:"admin_enquiry_triage_v1",convert:"admin_enquiry_convert_v1"}
export async function enquiryCommand(request:NextRequest,operation:EnquiryOperation) {
 ...
 try {
  ...
  const args=enquiryArgs(operation,body)
  if(!args) return reply("Check the fields, follow-up date and supporting note before saving.",400)
  const result=await backend().rpc<{status:string;id?:string}>(rpcs[operation],{...args,p_token:tokenHash(token),[operation==="create"?"p_key":"p_request"]:key})
  switch(result.status){
   case "success":return reply(operation==="convert"?"The work record has been created and linked to this enquiry.":"Saved. The enquiry history has been updated.",200,result.id)
   case "unauthorized":return reply("Your session has ended. Please sign in again.",401)
   case "conflict":return reply("This enquiry has changed. Reload it before trying again.",409)
   case "denied":return reply("Check that the enquiry is open and that the selected client and location are correct. Converted enquiries cannot be reopened here.",403)
   case "invalid":return reply("Check the form. Monitoring requests need a recorded terms-acceptance date that is not in the future.",400)
   default:return reply("We couldn’t confirm the change. Reload the enquiry before trying again.",503)
  }
 }catch{return reply("We couldn’t confirm the change. Reload the enquiry before trying again.",503)}
}
