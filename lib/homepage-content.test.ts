...
describe("homepage commercial copy", () => {
  ...
  it("names both specialist services and Relaunch Guard as Profile + Review monitoring", () => {
    ...
    expect(homepageGuard.lead).toContain("Early Access")
    expect(homepageGuard.lead.toLowerCase()).toContain("managed service")
    expect(homepageGuard.lead.toLowerCase()).toContain("self-serve dashboard")
    expect(homepageGuard.lead.toLowerCase()).not.toContain("no self-serve dashboard yet")
    expect(homepageGuard.cta).toBe("View Relaunch Guard pricing")
