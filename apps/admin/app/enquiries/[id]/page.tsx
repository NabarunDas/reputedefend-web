...
export default async function Enquiry({params}:{params:Promise<{id:string}>}){
 ...
 {e.status==="converted"?<section className="record-section"><h2>Linked work</h2><p>{e.caseRef?`Case ${e.caseRef}`:`Relaunch Guard request ${e.monitoringId}`}</p><p>Continue this work through the case or monitoring workflow. This enquiry stays linked for its history.</p></section>:['new','open','waiting'].includes(e.status) && <details className="record-section"><summary>Create work from this enquiry</summary><ConversionForm key={e.version} enquiry={e}/></details>}
