...
export default async function ActivityPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!filters) return <section className="page"><PageHeader title="Check your filters" description="That activity filter isn’t available." /><section className="panel"><Link href="/activity">Show all activity</Link></section></section>
  ...
  return <section className="page">
    <PageHeader title="Activity" description="Sign-ins, session changes, client records and enquiry activity. Times are shown in UK time." />
    <section className="panel">
      ...
      {!visible.length ? <EmptyState>No activity matches these filters.</EmptyState> : <div className="table-scroll" role="region" aria-label="Activity history" tabIndex={0}><table>
        ...
        <tbody>{visible.map(event => <tr key={event.id}><td><time dateTime={event.createdAt}>{ukDate(event.createdAt)}</time></td><td>{actions[event.action]}</td><td><Badge tone={event.outcome === "success" ? "success" : event.outcome === "denied" ? "danger" : "warning"}>{outcomes[event.outcome]}</Badge></td><td><details><summary>Reference {event.id}</summary><p className="reference">Request: {event.requestId}</p>{event.targetId && <p className="reference">{event.entity || "Session"}: {event.targetId}</p>}{event.reason && <p className="preserve-lines">{event.reason}</p>}{event.details && Object.keys(event.details).length > 0 && <pre className="reference">{JSON.stringify(event.details, null, 2)}</pre>}</details></td></tr>)}</tbody>
      </table></div>}
